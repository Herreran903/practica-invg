...
def plot_class_bars(
    ...
    ax.axhline(y=np.mean(recalls), color="red", linestyle="--", linewidth=1, label="Mean")
...
def plot_f1_bars_multilabel(
    ...
    ax.axhline(y=np.mean(f1_per_label), color="blue", linestyle="--", linewidth=1, label="Mean")
    plt.xticks(rotation=45, ha="right")
    ax.legend()
...
def plot_metrics_per_fold(
    ...
    ax.axhline(y=mean_val, color="red", linestyle="--", linewidth=1, label=f"Mean={mean_val:.3f}")
    ax.legend()
