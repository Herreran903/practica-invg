...
from ..jssp_images.evaluation import evaluate_multilabel as _evaluate_multilabel_base
from ..jssp_images.evaluation import (
    evaluate_regression,
)
...
def evaluate_classification_sat(
    ...
    valid_statuses = [
        s.lower() for s in sat_cfg.get("valid_statuses", ["ok", "sat", "unsat"])
    ]
    invalid_statuses = [
        s.lower()
        for s in sat_cfg.get(
            "invalid_statuses",
    ...
    pd.DataFrame(detail_rows).to_csv(
        os.path.join(fold_dir, f"fold{fold_idx}_resolved_detail.csv"), index=False
    )
