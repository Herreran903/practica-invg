"""
Generated dataset preparation for JSSP Tensors.
...
from .config_loader import JSPPTensorsConfig

# Import shared utilities from jssp_images
from ..jssp_images.jssp_instance_utils import generate_random_instance, save_instance_as_dzn
from ..jssp_images.minizinc_solver import execute_minizinc_solver, filter_solver_candidates


def prepare_generated_dataset(config: JSPPTensorsConfig) -> str:
    """
    ...
    print("=" * 75)
    print("GENERATED DATASET PREPARATION (Random Instances) - TENSORS")
    print("=" * 75)
        ...
        try:
            ...
            for time_limit_ms in time_limits_ms:
                ...
                for seed in random_seeds:
                    ...
                    solved_keys = [
                        k for k, s in results.items()
                        if s.get('solved_binary', 0) == 1
                    ]
                    
                    if solved_keys:
                        winner_key = min(
                            solved_keys,
                            key=lambda k: results[k]['runtime']
                        )
