...
def convert_raw_text_to_grayscale_image(
    raw_text_path: str, target_size: int = 128, normalize: bool = False
) -> np.ndarray:
    """
    Convert a text file into a grayscale image matrix.
    ...
    Optional:
    - If normalize=True, apply z-score normalization (x - mean) / std.
    ...
    5. Optionally apply z-score normalization (mean=0, std=1)

    Args:
        raw_text_path: Path to the input file
        target_size: Target size for the square output image (default: 128)
        normalize: If True, apply z-score normalization. Default: False

    Returns:
        Grayscale image as float32 numpy array of shape (target_size, target_size).
    ...
    # Convert to numpy array (float32), keep raw intensities [0, 255] by default
...
def convert_dataset_to_images(
    ...
    Default behavior (paper-like) saves images without z-score normalization
    (values ~ [0, 255] in float32). Set normalize=True to apply z-score.
        ...
        if not (isinstance(raw_path, str) and os.path.exists(raw_path)):
            ...
            if prefix_map:
                raw_path = resolve_path_with_prefix_map(
                    instances_root, instance_id, prefix_map
                )
            else:
                raw_path = None
        ...
        if raw_path and os.path.exists(raw_path):
            ...
            try:
                ...
                image_array = convert_raw_text_to_grayscale_image(
                    raw_path, target_size, normalize=normalize
                )

                # Generate unique filename using hash
                instance_id = str(
                    row.get(instance_id_col, row.get(instance_name_col, ""))
                )
    ...
    print(
        f"Conversion complete: {success_count} successful, {missing_count} missing/failed"
    )
    print("=" * 75)
...
def generate_all_images(
    ...
    Args:
        ...
        prefix_map: Optional mapping of prefixes to directory paths
    """
    convert_dataset_to_images(csv_path, instances_root, target_size, prefix_map)
